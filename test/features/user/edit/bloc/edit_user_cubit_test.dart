...
void main() {
  ...
  setUp(() {
    editUserCubit = EditUserCubit(mockUserApi);
  });

  blocTest<EditUserCubit, UiState<bool>>(
    "Given userApi, "
    "When update user method is called & it returns true, "
    "Then should emit Loading and Success state",
    build: () {
      // Stubbing
      when(() => mockUserApi.updateUser(any())).thenAnswer((_) async => true);

      return editUserCubit;
    },
    act: (bloc) => bloc.updateUser(mockUser),
    expect: () => [
      isA<Loading>(),
      isA<Success<bool>>().having((s) => s.data, 'data', true),
    ],
    verify: (_) {
      verify(() => mockUserApi.updateUser(any())).called(1);
    },
  );

  blocTest<EditUserCubit, UiState<bool>>(
    "Given userApi, "
    "When update user method is called & it returns false, "
    "Then should emit Loading and Error state",
    build: () {
      // Stubbing
      when(() => mockUserApi.updateUser(any())).thenAnswer((_) async => false);
